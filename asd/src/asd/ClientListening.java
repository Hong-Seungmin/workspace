...
public class ClientListening {
	public ClientListening(int portNumber, String fileName) {
		...
		byte[] buf = null;
		BufferedInputStream bufferdInput = null; // input 속도 향상을 위한
													// BufferedInputStream
		try {
		...
		try {
			dataInput = new DataInputStream(clientSocket.accept().getInputStream()); // 송신측에서
																						// 연결요청시
																						// accept
																						// 메소드에서
																						// 송신측과
			...
			if (totalSize <= 104857600) {
				i = 0; // 배열 인덱스 초기화
				buf = new byte[totalSize]; // 100MB보다 같거나 작으므로 totalSize로 배열크기
											// 다시 생성
				while (i < totalSize) {
					buf[i] = (byte) bufferdInput.read();
					i++; // 배열인덱스 이동
